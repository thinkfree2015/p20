...
import com.efeiyi.jh.plan.model.VirtualPlanElement;
    ...
    private List<VirtualPlanElement> getRelationAndComplete(ModelMap modelMap) throws Exception {
        ...
        if (!virtualPlanList.isEmpty()){
            for (VirtualPlan vp:virtualPlanList){
                ...
                //计划对象是--用户
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_USER)){
                    ...
                    vpe.setType(VirtualPlan.PLAN_TYPE_USER);
                }

                //计划对象是--订单
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_ORDER)){
                    ...
                    vpe.setType(VirtualPlan.PLAN_TYPE_ORDER);
                }

                //计划对象是--点赞
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_PRAISE)){
                    vpe.setType(VirtualPlan.PLAN_TYPE_PRAISE);
                }

                //计划对象是--商品
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_PRODUCT)){
                    vpe.setType(VirtualPlan.PLAN_TYPE_PRODUCT);
                }

                //计划对象是--收藏
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_COLLECT)){
                    vpe.setType(VirtualPlan.PLAN_TYPE_COLLECT);
                }

                //计划对象是--人气
                if (vp.getPlanType().equals(VirtualPlan.PLAN_TYPE_POPULARITY)){
                    vpe.setType(VirtualPlan.PLAN_TYPE_POPULARITY);
                }
